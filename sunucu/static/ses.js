 ...
 * ====================================================================
 *
 * ES MODÜLÜ. `index.html` bunu `type="module"` ile yüklüyor. Eski
 * klasik betikler (`app.js`) hâlâ `Ses.` diye çağırdığı için dosyanın
 * sonunda `window.Ses` köprüsü duruyor; o dosyalar da modüle dönünce
 * köprü silinecek.
 *
 * `@ts-check` açık: tipler JSDoc ile yazılıyor, derleme adımı YOK.
 * Denetim `npx tsc --noEmit -p sunucu/static` ile Windows'ta koşuyor,
 * Pi'ye giden dosya bu dosyanın kendisi.
 * ==================================================================== */
// @ts-check
...
/** @type {Record<string, {kay: AudioBufferSourceNode, k: GainNode, lfo: OscillatorNode} | null>} */
var akis = {};
 ...
 * @param {number} f0 @param {number} f1 @param {number} sure
 * @param {OscillatorType} [tip] @param {number} [tepe] @param {number} [gecikme] */
...
/** @param {string} ad @param {number} f0 @param {number} f1
 *  @param {number} q @param {number} tepe */
...
// Klasik betikler icin kopru — app.js modul olunca silinecek.
/** @type {any} */ (window).Ses = Ses;
