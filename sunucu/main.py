...
# --------------------------------------------------------------------------- #
# GÖRÜNTÜ İŞLEME KALDIRILDI — baştan kurulacak
#
# Burada `goruntu.py` (ExG, eşik, leke) ile `tespit.py`yi (kalibrasyon +
# karenin konumu) birleştiren çözümleme uçları vardı: /api/goruntu/coz,
# /fark, /cimlenme, /maske, /durum. Filiz bulma (`filiz.py`), bitki
# ölçümü (`bitkiolcum.py` + `gorus/`), tür tanıma (`tanima.py`) ve
# kalibrasyon kaydı (`kalibrasyon.py`) da birlikte silindi.
#
# GERİYE KALAN: kare çekme ve saklama (`kareler.py`, `arsiv.py`) ve
# kameranın kendisi (ajan). Yeni mimari bu ham karelerin üstüne
# kurulacak.
# --------------------------------------------------------------------------- #
...
def _bahce_veri() -> dict[str, Any]:
    ...
    # KALİBRASYON KAYDI KALDIRILDI. Panel bu iki alanı "kamera yatağa
    # oturuyor mu" diye okuyordu; şimdilik hep "hayır". Yeni mimari
    # gelince buraya kendi kaydını koyacak.
    kalib: dict[str, Any] = {}
    baslik = _baslik_oku()
