...
def _en_buyuk_dortgen(z):
    """Zarfın EN BÜYÜK ALANLI dörtgeni.

    Kap gerçekte dikdörtgen ama perspektifte yamuk görünüyor; en büyük
    alanlı dörtgen o yamuğu yakalıyor. En küçük alanlı döndürülmüş
    dikdörtgen, eğik kabın köşelerini kesip kenardaki fideleri dışarıda
    bırakıyordu. Zarf 60 noktaya seyreltiliyor — kaba kuvvet O(n^4).
    """
    n = len(z)
    if n < 4:
        return None
    if n > 60:
        adim = n / 60.0
        z = [z[int(i * adim)] for i in range(60)]
        n = len(z)
    en_iyi, alan_iyi = None, -1.0
    for i in range(n):
        for j in range(i + 1, n):
            for k in range(j + 1, n):
                for l in range(k + 1, n):
                    a = _ucgen(z[i], z[j], z[k]) + _ucgen(z[i], z[k], z[l])
                    if a > alan_iyi:
                        alan_iyi, en_iyi = a, [z[i], z[j], z[k], z[l]]
    return (en_iyi, alan_iyi) if en_iyi else None
...
def bul(rgb: np.ndarray) -> dict[str, Any] | None:
    ...
    d = _en_buyuk_dortgen(_zarf(list(zip(xs.tolist(), ys.tolist()))))
    if not d:
        return None
    kose, alan = d
